...
def hitung_hybrid_monitoring_v2(df_sales_raw, df_pribadi_raw, jurnal_data=None):
    """
    Engine V2 Resmi: Diperbarui dengan Logika Akuntansi Sub-Kategori Domestik
    dan Sistem Proteksi Saldo Bank Aktual (Anti-Bocor Data).
    """
    ...
    # ─────────────────────────────────────────────────────────────────
    # SINKRONISASI BARU: DAFTAR MAPPER SUB-KATEGORI PENGELUARAN DOMESTIK
    # ─────────────────────────────────────────────────────────────────
    ...
    # Inisialisasi keranjang akumulasi realisasi anggaran harian untuk UI Dashboard
    mutasi_pos_digital = {
    ...
    if not df_pribadi.empty and "Bank_Sumber" in df_pribadi.columns:
        for _, row in df_pribadi.iterrows():
            ...
            # --- EVALUASI UTAMA POTONG SALDO BANK & ISI KERANJANG ANGGARAN ---
            ...
            # Agregasi grafik anggaran harian secara dynamic mapping
            if kat in mutasi_pos_digital:
                mutasi_pos_digital[kat] += nominal

            # --- SENSOR DETEKSI OTOMATIS BIAYA OPERASIONAL KANTOR BISNIS ---
            ...
            # --- DETEKSI PEMBAYARAN TAGIHAN CC UNTUK DIKURANGI DARI OUTSTANDING ---
    ...
    if not df_sales.empty and "Sumber Dana" in df_sales.columns:
        ...
        df_cc_all = df_sales[mask_sumber_cc].copy()
            ...
            # --- PROSES AKHIR FILTER CUT-OFF KARTU KREDIT KULAKAN BISNIS ---
    ...
    # --- PERHITUNGAN MATEMATIKA FINAL BEBAN DAN DAYA TAHAN ---
    outstanding_cc_final = max(0.0, outstanding_cc_total - total_bayar_tagihan_cc)
    saldo_akhir_kas_fisik_toko = kas_setelah_investor - gaji_owner_dialokasikan - cadangan_bisnis_40_kertas
    ...
    # =========================================================================
    # RETURN DICTIONARY FINAL STANDAR CORPORATE (100% SINKRON UI)
    # =========================================================================
    return {
        "npm": npm,
        "roi": roi,
